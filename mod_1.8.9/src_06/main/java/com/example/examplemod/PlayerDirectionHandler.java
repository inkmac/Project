...
import com.example.examplemod.connection.ClientHandler;
...
public class PlayerDirectionHandler {
    public static boolean isPlayerDirectionActive = false;

    @SubscribeEvent
    public void onPlayerTick(TickEvent.PlayerTickEvent event) {
        if (!isPlayerDirectionActive) {
            return;
        }
        ...
        String data_64B = String.format("%-64s", data);

        ClientHandler.updateData(data_64B);
    }

    public String getPlayerFacing(EntityPlayer player) {
        // 获取玩家的朝向
